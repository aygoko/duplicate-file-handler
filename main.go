...
const (
	SortingOptionPrompt        = "Enter a sorting option:"
	CheckForDuplicatesPrompt   = "Check for duplicates?"
	DeleteFilesPrompt          = "Delete files?"
	FileFormatPrompt           = "Enter file format:"
	DirectoryNotSpecifiedError = "Directory is not specified"
	SizeSortingOption          = "Size sorting options:\n1. Descending\n2. Ascending"
	Yes                        = "yes"
	No                         = "no"
)

func main() {
	checkForArgument(os.Args)
	filesBySize := indexFiles(os.Args[1])
	scanner := bufio.NewScanner(os.Stdin)
	fileExt := readInput(FileFormatPrompt, scanner)
	filesBySize = selectExtension(filesBySize, fileExt)
	filesBySize = selectMultiple(filesBySize)
	fmt.Println(SizeSortingOption)
	option := readUserOption(scanner, SortingOptionPrompt, "1", "2")
	sizesSorted := sortSize(filesBySize, option)
	showFiles(filesBySize, sizesSorted)
	checkAnswer := readUserOption(scanner, CheckForDuplicatesPrompt, Yes, No)
	if Yes == checkAnswer {
		duplicateFiles := filterDuplicateFiles(filesBySize)
		orderedDuplicates := showDuplicateFiles(duplicateFiles, sizesSorted)
		deleteAnswer := readUserOption(scanner, DeleteFilesPrompt, Yes, No)
		if Yes == deleteAnswer {
...
func checkForArgument(args []string) {
	if len(args) < 2 {
		fmt.Println(DirectoryNotSpecifiedError)
		os.Exit(0)
	}
}
...
func readUserOption(scanner *bufio.Scanner, prompt string, options ...string) (option string) {
	for {
...
func showFiles(sizeIndex map[int][]string, sizesSorted []int) {
	for _, size := range sizesSorted {
